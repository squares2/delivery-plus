        ...
        grid.innerHTML = stores.map(s => {
            ...
            <div onclick="catalogOpenStore('${s.name.replace(/'/g,"\\'")}','${s.types[0]}')"
                ...
                <div style="width:52px;height:52px;border-radius:50%;overflow:hidden;background:var(--surface3);flex-shrink:0;position:relative;">
                    <img src="${delivoImg(`assets/${imgSlug}.webp`)}" alt="${s.name}"
                         style="width:100%;height:100%;object-fit:cover;"
                         onerror="if(this.src.endsWith('.webp')){this.src='${delivoImg(`assets/${imgSlug}.png`)}';}else{this.style.display='none';this.nextElementSibling.style.display='flex';}"
...
function _renderCatalogItems() {
    ...
    grid.innerHTML = searched.map(item => {
        ...
        const imgSrc  = png
            ? (_cpiLocalImagePreview[id] || delivoImg(`items2/${String(id).toLowerCase()}.webp${item.imgUpdatedAt ? '?v=' + item.imgUpdatedAt : ''}`))
            : '';
        return `
