...
صالح لمدة 5 دقائق. لا تشاركه مع أحد.`;
    ...
    /* ════════════════════════════════════════════════════════════
       GATE-FLOW REGISTRATION — phone + OTP, no username/password
       Only ever reached via openRegistrationFlow() when
       settings/launchGateEnabled is on. Reuses the same location
       picker (#reg-location-field, relocated in) and the same
       _sendOtpWhatsapp()/OTP-generation helpers as the classic flow
       above, but keeps its own OTP state, its own persistent
       send-attempt lockout, and its own account-creation call
       (DelivoAuth.registerViaGate) — phone number becomes the
       account's identifying value, no username or password involved.
    ════════════════════════════════════════════════════════════ */
    (function _initGateRegistration() {
        const GATE_OTP_SS_KEY = 'delivo_gate_otp_state';
        const GATE_OTP_TIMEOUT = 5 * 60 * 1000; // 5 minutes
        const GATE_OTP_MAX_SENDS = 3;
        const GATE_OTP_LOCKOUT_MS = 24 * 60 * 60 * 1000; // 24 hours
        const RTDB_BASE = 'https://deliveryonline-300f7-default-rtdb.firebaseio.com';

        const gateRegBtn      = document.getElementById('gatereg-submit');
        const gateResendBtn   = document.getElementById('gatereg-otp-resend-btn');
        const gateCancelBtn   = document.getElementById('gatereg-otp-cancel-btn');
        const gateOtpStep     = document.getElementById('gatereg-otp-step');
        const gateErrorEl     = document.getElementById('gatereg-error');
        const gatePhoneEl     = document.getElementById('gatereg-phone');
        if (!gateRegBtn) return;

        let _gatePhoneAvailable = true;
        let _gateOtpResendTimer = null;
        let _gateOtpExpireTimer = null;
        let _gateOtpSendInFlight = false;

        function _saveGateOtpState(state) { sessionStorage.setItem(GATE_OTP_SS_KEY, JSON.stringify(state)); }
        function _loadGateOtpState()      { try { return JSON.parse(sessionStorage.getItem(GATE_OTP_SS_KEY)||'null'); } catch(_){ return null; } }
        function _clearGateOtpState()     { sessionStorage.removeItem(GATE_OTP_SS_KEY); }

        function _startGateOtpCountdown(seconds) {
            const timerEl = document.getElementById('gatereg-otp-timer');
            if (gateResendBtn) gateResendBtn.disabled = true;
            clearInterval(_gateOtpResendTimer);
            let rem = seconds;
            _gateOtpResendTimer = setInterval(() => {
                rem--;
                if (timerEl) timerEl.textContent = `إعادة الإرسال متاحة بعد ${rem} ثانية`;
                if (rem <= 0) { clearInterval(_gateOtpResendTimer); if(timerEl)timerEl.textContent=''; if(gateResendBtn)gateResendBtn.disabled=false; }
            }, 1000);
        }
        function _startGateOtpExpireCountdown(expiresAt) {
            clearTimeout(_gateOtpExpireTimer);
            const _tick = () => {
                const left = expiresAt - Date.now();
                if (left <= 0) { _cancelGateOtpStep(); return; }
                const timerEl = document.getElementById('gatereg-otp-timer');
                const resendRunning = gateResendBtn?.disabled;
                if (timerEl && !resendRunning) {
                    const m = Math.floor(left/60000), s = Math.floor((left%60000)/1000);
                    timerEl.textContent = `⏰ ينتهي الكود بعد ${m}:${s.toString().padStart(2,"0")}`;
                }
                _gateOtpExpireTimer = setTimeout(_tick, 1000);
            };
            _gateOtpExpireTimer = setTimeout(_tick, 1000);
            setTimeout(_cancelGateOtpStep, Math.max(0, expiresAt - Date.now()));
        }
        function _cancelGateOtpStep() {
            clearInterval(_gateOtpResendTimer); clearTimeout(_gateOtpExpireTimer); _clearGateOtpState();
            if (gateOtpStep)   gateOtpStep.style.display   = 'none';
            if (gateCancelBtn) gateCancelBtn.style.display = 'none';
            if (gateRegBtn)    gateRegBtn.textContent       = 'إرسال كود التحقق';
            const timerEl = document.getElementById('gatereg-otp-timer');
            if (timerEl) timerEl.textContent = '';
            showError(gateErrorEl, '⌛ انتهت صلاحية كود التحقق. يمكنك إرسال كود جديد.');
        }

        // ── Persistent send-attempt lockout (per device, survives page
        // reloads/closed tabs — unlike the classic flow's 30s in-session
        // cooldown, this needs to last a full day) ──────────────────
        async function _getOtpAttemptState(uuid) {
            try {
                const r = await fetch(`${RTDB_BASE}/otpAttempts/${uuid}.json`);
                return r.ok ? (await r.json()) : null;
            } catch (_) { return null; }
        }
        async function _recordOtpAttempt(uuid, state) {
            try {
                await fetch(`${RTDB_BASE}/otpAttempts/${uuid}.json`, {
        ...
        // ── Live phone-availability check (mirrors the classic form's) ──
        if (gatePhoneEl) {
            gatePhoneEl.addEventListener('input', () => {
                clearTimeout(gatePhoneEl._checkTimer);
                const digits = gatePhoneEl.value.replace(/[\s\-]/g, '');
                const hint   = gatePhoneEl.closest('.modal-field')?.querySelector('.field-hint');
                if (!digits || digits.length < 7) {
                    _gatePhoneAvailable = true;
                    _setFieldState(gatePhoneEl, 'idle', hint, 'مثال: 03 123 456 أو 71 123 456');
                    return;
                }
                if (!/^(03|70|71|76|78|79|81|82|83|86)\d{6}$/.test(digits)) {
                    _gatePhoneAvailable = false;
                    _setFieldState(gatePhoneEl, 'error', hint, 'رقم لبناني غير صحيح. مثال: 71 123 456');
                    return;
                }
                _setFieldState(gatePhoneEl, 'loading', hint, '⏳ جاري التحقق…');
                gatePhoneEl._checkTimer = setTimeout(async () => {
                    try {
                        const [phResp, limitResp] = await Promise.all([
                            fetch(`${RTDB_BASE}/phoneIndex/${digits}.json`),
                            fetch(`${RTDB_BASE}/settings/maxAccountsPerPhone.json`),
                        ]);
                        const data  = await phResp.json();
                        const limit = Math.max(1, parseInt(await limitResp.json()) || 1);
                        let phCount = 0;
                        if (data !== null) phCount = (typeof data === 'object' && !Array.isArray(data)) ? Object.keys(data).length : 1;
                        if (phCount >= limit) {
                            _gatePhoneAvailable = false;
                            _setFieldState(gatePhoneEl, 'error', hint, '❌ هذا الرقم مسجّل مسبقاً. استخدم رقماً آخر أو سجّل دخولك');
                        } else {
                            _gatePhoneAvailable = true;
                            _setFieldState(gatePhoneEl, 'success', hint, '✅ الرقم متاح');
                        }
                    } catch (_) {
                        _gatePhoneAvailable = true;
                        _setFieldState(gatePhoneEl, 'idle', hint, 'مثال: 03 123 456 أو 71 123 456');
        ...
        async function _doSendGateOtp(phoneDigits, fullname, lat, lng) {
            ...
            const attemptState = await _getOtpAttemptState(uuid);
            if (attemptState?.lockedUntil && Date.now() < attemptState.lockedUntil) {
                const hoursLeft = Math.ceil((attemptState.lockedUntil - Date.now()) / (60 * 60 * 1000));
                showError(gateErrorEl, `⛔ تجاوزت الحد المسموح لإرسال كود التحقق (${GATE_OTP_MAX_SENDS} محاولات). حاول مجدداً بعد ${hoursLeft} ساعة.`);
                return false;
            }
            const sendsSoFar = (attemptState && (!attemptState.lockedUntil || Date.now() >= attemptState.lockedUntil)) ? (attemptState.count || 0) : 0;
            if (sendsSoFar >= GATE_OTP_MAX_SENDS) {
                const lockedUntil = Date.now() + GATE_OTP_LOCKOUT_MS;
                await _recordOtpAttempt(uuid, { count: sendsSoFar, lockedUntil });
                showError(gateErrorEl, `⛔ تجاوزت الحد المسموح لإرسال كود التحقق (${GATE_OTP_MAX_SENDS} محاولات). حاول مجدداً بعد 24 ساعة.`);
                return false;
            }

            if (_gateOtpSendInFlight) return false;
            _gateOtpSendInFlight = true;
            setLoading(gateRegBtn, true, '⏳ جاري الإرسال...');
            try {
                const code      = await _sendOtpWhatsapp(phoneDigits);
                const expiresAt = Date.now() + GATE_OTP_TIMEOUT;
                _saveGateOtpState({ fullname, phone: phoneDigits, lat, lng, locationSource: window._regLocationSource || null, code, expiresAt });
                await _recordOtpAttempt(uuid, { count: sendsSoFar + 1, lockedUntil: null });

                if (gateOtpStep) {
                    gateOtpStep.style.display = 'block';
                    const hintEl = document.getElementById('gatereg-otp-hint');
                    if (hintEl) hintEl.textContent = `تم إرسال كود إلى واتساب رقم 961${_toIntlPhone(phoneDigits)}`;
                }
                if (gateCancelBtn) gateCancelBtn.style.display = 'flex';
                gateRegBtn.disabled = false;
                gateRegBtn.textContent = 'تأكيد الكود وإنشاء الحساب';
                _startGateOtpCountdown(60);
                _startGateOtpExpireCountdown(expiresAt);
                document.getElementById('gatereg-otp')?.focus();

                const remaining = GATE_OTP_MAX_SENDS - (sendsSoFar + 1);
                if (remaining > 0 && remaining < GATE_OTP_MAX_SENDS) {
                    showError(gateErrorEl, '');
                    hideError(gateErrorEl);
                }
                _gateOtpSendInFlight = false;
                return true;
            } catch (e) {
                showError(gateErrorEl, e.message);
                setLoading(gateRegBtn, false, 'إرسال كود التحقق');
                _gateOtpSendInFlight = false;
                return false;
            }
        }
        ...
        if (gateRegBtn) {
            gateRegBtn.addEventListener('click', async () => {
                const fullname    = document.getElementById('gatereg-displayname')?.value.trim() || '';
                const phoneRaw    = document.getElementById('gatereg-phone')?.value || '';
                const phoneDigits = phoneRaw.replace(/[\s\-]/g, '');

                const saved            = _loadGateOtpState();
                const isOtpConfirmStep = saved && gateOtpStep?.style.display !== 'none';

                if (!isOtpConfirmStep) {
                    if (!fullname) { showError(gateErrorEl, 'الاسم الكامل مطلوب'); return; }
                    if (!phoneDigits) { showError(gateErrorEl, 'رقم الهاتف مطلوب. أدخل رقمك اللبناني.'); return; }
                    if (!/^(03|70|71|76|78|79|81|82|83|86)\d{6}$/.test(phoneDigits)) {
                        showError(gateErrorEl, 'رقم الهاتف غير صحيح. مثال: 03 123 456 أو 71 123 456'); return;
                    }
                    if (!_gatePhoneAvailable) { showError(gateErrorEl, 'هذا الرقم مسجّل مسبقاً. استخدم رقماً آخر أو سجّل دخولك'); gatePhoneEl?.focus(); return; }
                    if (!_requireRegLocation()) { showError(gateErrorEl, '📍 يجب تحديد موقعك لإتمام إنشاء الحساب'); return; }

                    const lat = parseFloat(document.getElementById('reg-lat')?.value);
                    const lng = parseFloat(document.getElementById('reg-lng')?.value);
                    if (typeof window._checkCoverageOrWarn === 'function') {
                        const insideCoverage = await window._checkCoverageOrWarn(lat, lng, () => {
                            document.getElementById('reg-location-status')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                            document.getElementById('reg-location-map')?.click();
                        }, (newLat, newLng) => {
                            document.getElementById('reg-lat').value = newLat;
                            document.getElementById('reg-lng').value = newLng;
                            window._regLocationSource = 'map';
                            setLocationStatus('success', '✓ تم تحديث موقعك — تابع التسجيل الآن');
                            if (window._regMap) { window._regMap.remove(); window._regMap = null; window._regMarker = null; }
                    ...
                    hideError(gateErrorEl);
                    await _doSendGateOtp(phoneDigits, fullname, lat, lng);
                    return;
                }

                // Step 2 — verify code + create account
                const entered = document.getElementById('gatereg-otp')?.value.trim() || '';
                if (!entered)      { showError(gateErrorEl, 'أدخل كود التحقق المُرسَل على واتساب'); return; }
                if (!saved?.code)  { showError(gateErrorEl, 'انتهت صلاحية الكود. اضغط إعادة الإرسال'); return; }
                if (Date.now() > saved.expiresAt) { _cancelGateOtpStep(); return; }
                if (entered !== saved.code) { showError(gateErrorEl, '❌ الكود غير صحيح. تحقق من واتساب وحاول مجدداً'); document.getElementById('gatereg-otp')?.select(); return; }

                if (_gateOtpSendInFlight) return;
                _gateOtpSendInFlight = true;
                setLoading(gateRegBtn, true, '⏳ جاري إنشاء الحساب...');
                let result;
                try {
                    result = await window.DelivoAuth.registerViaGate({
                        fullname: saved.fullname, phone: saved.phone,
                        lat: saved.lat, lng: saved.lng, locationSource: saved.locationSource,
                    });
                } catch (e) {
                    result = { error: true, message: e?.message || 'تعذر الاتصال بالخادم. تحقق من شبكتك وحاول مجدداً' };
                }
                _gateOtpSendInFlight = false;
                setLoading(gateRegBtn, false, 'تأكيد الكود وإنشاء الحساب');
                if (result.error) {
                    showError(gateErrorEl, result.message + ' — الكود ما زال صالحاً، اضغط "تأكيد" مجدداً بدون طلب كود جديد.');
                } else {
                    _clearGateOtpState(); clearInterval(_gateOtpResendTimer); clearTimeout(_gateOtpExpireTimer);
                    closeModal('modal-gate-register');
                    clearFields(['gatereg-displayname','gatereg-phone','gatereg-otp']);
                    resetLocationBtn();
                    _relocateLocationField(false);
                    if (gateOtpStep)   gateOtpStep.style.display   = 'none';
                    if (gateCancelBtn) gateCancelBtn.style.display = 'none';
                    gateRegBtn.textContent = 'إرسال كود التحقق';
                }
            });
        }
        ...
        if (gateResendBtn) {
            gateResendBtn.addEventListener('click', async () => {
                const state = _loadGateOtpState();
                if (!state) return;
                await _doSendGateOtp(state.phone, state.fullname, state.lat, state.lng);
            });
        }
        ...
        document.getElementById('gatereg-signin-link')?.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            closeModal('modal-gate-register');
            setTimeout(() => openSigninFlow(), 180);
        });
    })();
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.acct-btn-register')) {
            closeModal('modal-account');
            setTimeout(() => openRegistrationFlow(), 180);
            return;
        }
    });

    // Login modal's "سجّل الآن" link — also routed through the gate-aware
    // helper. Handled with its own direct listener (not the delegated one
    // above) so stopPropagation() reliably prevents the generic
    // [data-modal] click handler from opening #modal-subscribe directly.
    document.getElementById('modal-login-register-link')?.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        closeModal('modal-login');
        setTimeout(() => openRegistrationFlow(), 180);
...
function renderAccountModal() {
    ...
    if (user) {
        ...
        const isGateAccount = user.registrationMethod === 'gate-otp';
        if (avatarEl) avatarEl.textContent = initial;
        if (nameEl)   nameEl.textContent   = user.displayName || user.username || 'User';
        if (emailEl) {
            // Gate-flow accounts have no username handle to show (their
            // phone number IS the account, but it isn't a "@handle") —
            // show the phone itself instead of "@<phone digits>".
            emailEl.textContent = isGateAccount
                ? (user.phone ? user.phone.replace('+961', '').replace(/\s/g, '') : '')
        ...
        // Gate-otp accounts have no username/password at all — hide the
        // password-change section in Edit Profile, AND the "change
        // password" row in this account menu itself.
        const pwSection = document.getElementById('edit-password-section');
        if (pwSection) pwSection.style.display = isGateAccount ? 'none' : '';
        const pwRow = document.getElementById('acct-password-btn');
        if (pwRow) pwRow.style.display = isGateAccount ? 'none' : '';
    } else {
...
/* ════════════════════════════════════════════════════════════
   Launch-gate integration — new-visitor pre-registration capture
   (see scripts/launch-gate.js). When that gate has already collected
   a full name + phone for this device, auto-fill them the moment the
   official registration modal opens, so the visitor doesn't have to
   retype anything. Uses a MutationObserver on the modal's own class
   attribute rather than hooking openModal()/[data-modal] triggers
   directly — this codebase has more than one same-named openModal()
   declaration across files, so observing the actual DOM state is the
   one approach guaranteed to work regardless of which wins.
════════════════════════════════════════════════════════════ */
// Moves the shared location-picker block (#reg-location-field — map, GPS,
// coverage-check, everything) between its home slot in #modal-subscribe
// and the placeholder slot in #modal-gate-register. Reusing the exact
// same DOM node (not a clone) means there's only ever one copy of that
// fairly involved logic to maintain, and only one live Leaflet instance
// at a time regardless of which modal is showing.
function _relocateLocationField(toGateFlow) {
    const field = document.getElementById('reg-location-field');
    const dest  = document.getElementById(toGateFlow ? 'gatereg-location-slot' : 'reg-location-home-slot');
    if (field && dest && field.parentElement !== dest) {
        dest.appendChild(field);
    }
}

// Decides what "sign in" should open: the classic username/password
// login (gate feature off) or the lightweight phone re-confirmation
// modal (gate feature on — gate-otp accounts have no username/password
// to log in with). That phone-check modal already falls through to full
// registration on its own if the phone typed doesn't match anything on
// this device, so a genuinely new phone number is still free to sign up.
async function openSigninFlow() {
    let gateOn = false;
    try {
        const r = await fetch('https://deliveryonline-300f7-default-rtdb.firebaseio.com/settings/launchGateEnabled.json');
        const v = r.ok ? await r.json() : null;
        gateOn = (v === true || v === 'true');
    } catch (_) {}
    if (gateOn) {
        // Always start this modal fresh — its error banner and phone field
        // otherwise persist across separate attempts (opening a modal only
        // toggles its visibility, it doesn't reset anything inside it),
        // which could make a stale error from a previous try look like an
        // immediate response to this one.
        const errEl = document.getElementById('gatecheck-error');
        if (errEl) { errEl.style.display = 'none'; errEl.textContent = ''; }
        const phoneEl = document.getElementById('gatecheck-phone');
        if (phoneEl) phoneEl.value = '';
    }
    openModal(gateOn ? 'modal-gate-phone-check' : 'modal-login');
}

// Decides which registration modal to open: the new phone+OTP gate flow
// (when the admin has enabled settings/launchGateEnabled) or the classic
// username/password modal (default / feature off). The classic modal's
// behavior is completely untouched either way — this only decides which
// one gets shown.
async function openRegistrationFlow() {
    ...
    _relocateLocationField(gateOn);
    openModal(gateOn ? 'modal-gate-register' : 'modal-subscribe');
}

// If the gate-flow modal is dismissed (close button / backdrop / Escape —
// all still work normally on this one) without completing registration,
// move the location field back home so #modal-subscribe isn't left
// without it if it's opened directly afterward (e.g. admin disables the
// gate feature mid-session, or a returning classic user logs in instead).
(function _watchGateRegisterClose() {
    const modal = document.getElementById('modal-gate-register');
    if (!modal) return;
    const observer = new MutationObserver(() => {
        if (!modal.classList.contains('active')) _relocateLocationField(false);
    });
    observer.observe(modal, { attributes: true, attributeFilter: ['class'] });
})();

(function _initGatePhoneCheck() {
    const btn     = document.getElementById('gatecheck-submit');
    const phoneEl = document.getElementById('gatecheck-phone');
    const errorEl = document.getElementById('gatecheck-error');
    if (!btn) return;

    document.getElementById('gatecheck-classic-login-link')?.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        closeModal('modal-gate-phone-check');
    ...
    btn.addEventListener('click', async () => {
        ...
        if (!/^(03|70|71|76|78|79|81|82|83|86)\d{6}$/.test(digits)) {
            showError(errorEl, 'رقم لبناني غير صحيح. مثال: 71 123 456');
            return;
        }
        setLoading(btn, true, '⏳ جاري التحقق...');
        let result;
        try {
            result = await window.DelivoAuth.checkReturningGateUser({ phone: digits });
        } catch (_) {
            result = { matched: false };
        }
        setLoading(btn, false, 'متابعة');

        if (result.matched) {
            closeModal('modal-gate-phone-check');
            clearFields(['gatecheck-phone']);
            hideError(errorEl);
            // Signed back in — but deliberately does NOT auto-resume/place
            // the order on its own. An order must only ever be sent by the
            // customer's own explicit tap on "إرسال الطلب", never as an
            // automatic side effect of anything else (a stale cart from
            // an earlier session, prices having since changed, etc. make
            // silently completing a purchase here unsafe).
            if (typeof _showToast === 'function') {
                _showToast('✓ تم تسجيل دخولك — اضغط "إرسال الطلب" لإتمام طلبك', 'success');
            }
        } else {
            showError(errorEl, 'لم نتعرّف على هذا الرقم على هذا الجهاز. أكمل التسجيل للمتابعة.');
            setTimeout(() => {
                closeModal('modal-gate-phone-check');
                clearFields(['gatecheck-phone']);
                hideError(errorEl);
    ...
    phoneEl?.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); btn.click(); } });
})();

function _wireLaunchGateAutofill(modalId, nameFieldId, phoneFieldId) {
    ...
    const applyAutofill = () => {
        ...
        const cachedName  = localStorage.getItem('delivo_launch_fullname');
        const cachedPhone = localStorage.getItem('delivo_launch_phone');

        // Never overwrite something the visitor already typed themselves.
        if (nameEl && !nameEl.value && cachedName) {
            nameEl.value = cachedName;
        }
        if (phoneEl && !phoneEl.value && cachedPhone) {
            phoneEl.value = cachedPhone;
            // Trigger the existing live phone-availability check/styling.
            phoneEl.dispatchEvent(new Event('input', { bubbles: true }));
        }
    };

    const observer = new MutationObserver(() => {
        if (modal.classList.contains('active')) applyAutofill();
    });
    observer.observe(modal, { attributes: true, attributeFilter: ['class'] });
}
_wireLaunchGateAutofill('modal-subscribe',     'reg-displayname',     'reg-phone');
_wireLaunchGateAutofill('modal-gate-register', 'gatereg-displayname', 'gatereg-phone');